...
2026-06-23 (2): **Maintenance session — AT export ticket, PayPal perf, lint.** (1) Drafted a paste-ready **new** Jira case to schedule the AutoTranslate usage export ([[autotranslate-export-new-ticket]]); advised to **comment first, then open the ticket** — comment posted, ticket pending a response. (2) Deleted stray 0-byte `ImmoScout24.md` at repo root. (3) Closed the PayPal LOW perf item: memoized `ppGetGatewayInfo` by payment string (`ppGwCache`, cleared at top of `ppRefresh`) in `.raw/Spreedly_Conversion.html` — collapses repeated O(entries × gateways) substring scans; behavior identical. (4) Ran a 316-page lint ([[lint-report-2026-06-23]]): vault healthy, 0 frontmatter gaps, no address issues. **Lesson: the lint scanner matches wikilinks by basename only and misses aliases** → 3 of 4 flagged dead-link clusters were false positives (EMH / Marcos López de Prado / ImmoScout24 all alias-resolved). One real dead link fixed: dropped the `?` from `[[How does the LLM Wiki pattern work?]]` in 3 concept pages. Playbook: [[2026-06-23-at-ticket-paypal-perf-and-lint]].

2026-06-23: **AutoTranslate data answers + Conversion Dashboard deploy-lag.** (1) The Trial Conversion Dashboard "missing changes" report was a **stale Netlify deploy**, not a code bug — the downloaded `index (2).html` is byte-identical (same MD5) to `.raw/Conversion_Dashboard.html`, both already contain the cohort-baseline code (`TRACKING_START='2026-04'`, the `created_at>='2026-04'` filter, the "Since April 2026" badge), and rendering the download live produced the correct 81-shop / 6.2% view. The 28%/118-shop view is an older build still on Netlify; the user's link was old. `.raw/Conversion_Dashboard.html` is itself only a "rescue from Netlify" snapshot, NOT the edit source. (2) **AutoTranslate data finally arrived** (Marion email + 2 Confluence pages) — folded into [[Questions Auto Translate]]. No automated feed today, but an export script already exists unscheduled: `usageAutoTranslation.pl -allstores` → per-store usage CSV. Per-ShopType budgets captured (Cloud/Direct NowM 1M/NowXL 5M; others NowM 500K/NowXL 1M), AddOn raises cap to 25M; DeepL = 4.99€/mo + 20€/1M chars; booked-AddOn + tracked-usage shop lists recorded. Drafted [[UNITY-9917-export-request-draft]] to get R&D to schedule the export. **Claude has no Confluence access (org policy) — user pastes page content.** Playbook: [[2026-06-23-autotranslate-data-and-dashboard-deploy]].
