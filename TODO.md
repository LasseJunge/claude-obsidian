...
- [x] **Resolve conflict: how many PayPal versions are being shut down?** — Chris said **2**, Karsten said **3**. This decides which payment values get classified `shutdown` in the PayPal Gateways modal, and therefore the "On Shutdown Versions" KPI + migration target. Karsten owns the data (provides the CSV) so likely authoritative, but confirm the exact version names with both before finalizing gateway config. Candidate shutdown versions in the data: PayPal (1,082), PayPalPro (17), PayPalIntegralEvolution (7). **Resolved 2026-06-30: 3 versions — all three (PayPal, PayPalPro, PayPalIntegralEvolution) are being shut down (Karsten authoritative). Action: mark all three `shutdown` in the PayPal Gateways modal (runtime config in Supabase, not code).**
